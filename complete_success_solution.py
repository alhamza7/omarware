...
def create_sap_data_complete_success():
    """الحل الكامل الناجح: إنشاء بيانات SAP"""
    try:
        ...
        print("=== الحل الكامل الناجح: إنشاء بيانات SAP ===")
        ...
        # 2. Get default UoM ID
        print("2. الحصول على وحدة القياس الافتراضية...")
        cur.execute("SELECT id FROM uom_uom WHERE name->>'en_US' = 'Units' LIMIT 1;")
        uom_result = cur.fetchone()
        if uom_result:
            default_uom_id = uom_result[0]
            print(f"   ✅ وحدة القياس الافتراضية: {default_uom_id}")
        else:
            # Get any UoM
            cur.execute("SELECT id FROM uom_uom LIMIT 1;")
            uom_result = cur.fetchone()
            if uom_result:
                default_uom_id = uom_result[0]
                print(f"   ✅ وحدة القياس: {default_uom_id}")
            else:
                print("   ❌ لم يتم العثور على وحدة قياس")
                return False
        
        # 3. Create products with all required fields
        print("3. إنشاء المنتجات...")
        sample_products = [
        ...
        for product_name, sap_code in sample_products:
            try:
                ...
                if existing:
                    ...
                    # Create product with all required fields including publish_date
                    cur.execute("""
                        INSERT INTO product_template (
                            name, type, uom_id, service_tracking, tracking, publish_date,
                            create_uid, write_uid, create_date, write_date
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())
                        RETURNING id;
                    """, (f'{{"en_US": "{product_name}"}}', 'product', default_uom_id, 'no', 'none', '2025-10-16', 2, 2))
        ...
        # 4. Update connector statistics
        print("4. تحديث إحصائيات المزامنة...")
        cur.execute("""
...
if __name__ == "__main__":
    success = create_sap_data_complete_success()
    if success:
        print("\n🎉 تمت المزامنة بنجاح!")
