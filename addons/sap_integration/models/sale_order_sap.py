                    ...
                    if quotation.sap_doc_entry and quotation.sap_doc_entry > 0:
                        ...
                        if is_existing_order:
                            ...
                            if is_existing_quotation:
                                ...
                                try:
                                    # ─── الخطوة 1: تنظيف الـ Quotation في SAP باستخدام update_quotation ───
                                    # نُمرِّر existing_odoo_lines لكي يُطابق SAP الـ LineNums الحاليين
                                    # ويحذف أي سطور زائدة (بدل إضافة سطور مكررة)
                                    odoo_lines_for_patch = [
                                        {
                                            'item_code': l.get('ItemCode', ''),
                                            'sap_line_num': None,  # سيُجلب من SAP في update_quotation
                                        }
                                        for l in quotation_data.get('DocumentLines', [])
                                        if l.get('ItemCode')
                                    ]
                                    patch_result = connection.update_quotation(
                                        quotation.sap_doc_entry,
                                        quotation_data,
                                        existing_odoo_lines=odoo_lines_for_patch,
                                    )
                                    if patch_result is None:
                                        _logger.warning(f"⚠️ update_quotation returned None for {quotation.name} (DocEntry={quotation.sap_doc_entry}), proceeding with conversion anyway")
                                    else:
                                        _logger.info(f"✅ Quotation cleaned and updated successfully before conversion")

                                    # ─── الخطوة 2: جلب LineNums الفعلية من SAP بعد PATCH ───
                                    # BaseLine يجب أن يكون LineNum الحقيقي في SAP وليس index Python
                                    sap_line_num_map = {}  # ItemCode -> LineNum (من SAP)
                                    try:
                                        fetch_url = (
                                            f"{connection.base_url}/Quotations({quotation.sap_doc_entry})"
                                            f"?$select=DocumentLines"
                                        )
                                        fetch_resp = connection.session.get(fetch_url, headers=headers, timeout=30)
                                        if fetch_resp.status_code == 200 and fetch_resp.content:
                                            fetched_doc = fetch_resp.json()
                                            for sap_line in fetched_doc.get('DocumentLines', []):
                                                item_code = sap_line.get('ItemCode', '')
                                                line_num = sap_line.get('LineNum')
                                                line_status = sap_line.get('LineStatus', '')
                                                # الاحتفاظ بأول سطر مفتوح لكل ItemCode
                                                if (item_code and line_num is not None
                                                        and line_status != 'bost_Close'
                                                        and item_code not in sap_line_num_map):
                                                    sap_line_num_map[item_code] = line_num
                                            _logger.info(f"[SAP] Fetched LineNum map from Quotation (post-PATCH): {sap_line_num_map}")
                                        else:
                                            _logger.warning(
                                                f"⚠️ Could not fetch Quotation lines from SAP "
                                                f"(status={fetch_resp.status_code}), will use enumerate index as fallback"
                                            )
                                    except Exception as fetch_err:
                                        _logger.warning(f"⚠️ Failed to fetch Quotation lines for LineNum mapping: {fetch_err}")

                                    # ─── الخطوة 3: بناء سطور الـ Sales Order مع BaseLine الصحيح ───
                                    order_lines = []
                                    for idx, line in enumerate(quotation_data.get('DocumentLines', [])):
                                        ...
                                        if item_code and item_code in sap_line_num_map:
                                            order_line['BaseLine'] = sap_line_num_map[item_code]
                                            _logger.info(f"[SAP] BaseLine for ItemCode={item_code}: {sap_line_num_map[item_code]}")
                                        else:
                                            _logger.warning(
                                                f"⚠️ LineNum not found in SAP map for ItemCode='{item_code}', "
                                                f"using enumerate index={idx} as fallback (may cause -2010)"
                                            )
