...
class ResPartner(models.Model):
    ...
    def _prepare_partner_data_for_sap(self, partner, connection=None):
        """إعداد بيانات partner للـ SAP Service Layer"""
        partner_data = {
            'CardName': partner.name or '',
            'CardType': 'C',  # C = Customer
        }
        
        # إذا كان partner لديه ref (CardCode) بالفعل ومُزامن مع SAP، نستخدمه للتحديث
        if partner.ref and partner.sap_synced:
            partner_data['CardCode'] = partner.ref
            _logger.info(f"Using existing CardCode for update: {partner.ref}")
        else:
            # إرسال CardCode مباشرة - يبدأ من IBG06666
            # محاولة الحصول على آخر CardCode من SAP وإضافة 1، مع ضمان أن يبدأ من 6666 كحد أدنى
            card_code = "IBG06666"  # القيمة الافتراضية
            if connection:
                try:
                    last_card_code = connection.get_next_card_code('IBG')
                    if last_card_code:
                        # استخراج الرقم من CardCode وإضافة 1
                        import re
                        match = re.search(r'IBG(\d+)', last_card_code)
                        if match:
                            last_number = int(match.group(1))
                            # استخدام max لضمان أننا نبدأ من 6666 كحد أدنى
                            next_number = max(last_number + 1, 6666)
                            card_code = f"IBG{str(next_number).zfill(5)}"
                            _logger.info(f"Last CardCode from SAP: {last_card_code}, next will be: {card_code}")
                        else:
                            _logger.info(f"Could not extract number from {last_card_code}, using default: {card_code}")
                    else:
                        _logger.info(f"No last CardCode found, using default: {card_code}")
                except Exception as e:
                    _logger.warning(f"Error getting last CardCode from SAP: {e}, using default: {card_code}")
            
            partner_data['CardCode'] = card_code
            
            # الحصول على Series ID من SAP (كما في الكود PHP)
            if connection:
                try:
                    series_number = connection.get_ibg_series_number()
                    partner_data['Series'] = series_number
                    _logger.info(f"Generated CardCode: {card_code}, Series: {series_number}")
                except Exception as e:
                    _logger.warning(f"Error getting Series number: {e}, continuing without Series")
                    _logger.info(f"Generated CardCode: {card_code} (without Series)")
            else:
                _logger.info(f"Generated CardCode: {card_code} (no connection for Series)")
        ...
        # إضافة العنوان
        if partner.street:
            partner_data['Address'] = partner.street
        ...
        if partner.country_id:
            partner_data['Country'] = partner.country_id.code or ''
