...
class PosPerfumeController(http.Controller):
    ...
    def get_product_data(self, product_id, pricelist_id=None, uom_id=None, warehouse_id=None):
        ...
        try:
            ...
            # Default to the product's own UoM price on initial load so piece-based
            # products never inherit a dozen/carton price by mistake.
            effective_base_uom = self._get_effective_base_uom(product)
            default_uom_id = int(uom_id) if uom_id else effective_base_uom.id
            default_price = self._get_default_price_from_uoms(available_uoms, default_uom_id)
            ...
            return {
                ...
                'data': {
                    ...
                    'product_uom_id': effective_base_uom.id,
                    'product_uom_name': effective_base_uom.name,
    ...
    def _get_effective_base_uom(self, product):
        """Return the best non-generic base UoM for POS pricing and display."""
        ...
        if extended_info.inventory_uom_id and extended_info.inventory_uom_id.id != self.SAP_GENERIC_UOM_ID:
            return extended_info.inventory_uom_id
        if extended_info.sales_uom_id and extended_info.sales_uom_id.id != self.SAP_GENERIC_UOM_ID:
    ...
    def _get_uoms_from_pricelist(
        ...
        effective_base_uom = self._get_effective_base_uom(product)
        if not pricelist:
            return [{'id': effective_base_uom.id, 'name': effective_base_uom.name, 'price': product.list_price}]
        ...
        if effective_base_uom.id not in uoms_with_prices:
            # Never copy the highest available pack price onto the base UoM. If the
            # base UoM has no explicit price row, keep it visible with its own
        ...
        for item in items:
            ...
            if item.product_uom_id and item.product_packaging_id:
                ...
                if raw_uom.id == self.SAP_GENERIC_UOM_ID:
                    if effective_base_uom.id != self.SAP_GENERIC_UOM_ID:
                        uom = effective_base_uom
                    else:
                        uom = raw_uom
                else:
                    uom = raw_uom
    ...
    def _get_default_price_from_uoms(self, available_uoms, default_uom_id=None):
        ...
        - Prefer the exact requested UoM price whenever possible.
        - Fall back to the highest non-zero price only when the requested UoM is absent.
        """
        ...
        # Exact UoM match requested (e.g. initial product load or onchange_uom)
        if default_uom_id:
            for uom in available_uoms:
                if uom['id'] == default_uom_id and uom['price'] > 0:
                    return uom['price']
            for uom in available_uoms:
                if uom['id'] == default_uom_id:
                    return uom['price']
            # UoM not present in list → fall through to highest
...
شكراً لتعاملك معنا! 💚"""
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # SAP Document Chain Status
    # Returns: quotation → sales_order → delivery → ar_invoice
    # ─────────────────────────────────────────────────────────────────────────
    @http.route('/pos_perfume/get_sap_chain', type='json', auth='user')
    def get_sap_chain(self, sap_doc_entry=None, pos_order_id=None):
        """
        Fetch SAP document chain for a given Sales Order (by DocEntry).
        Returns stage: 'quotation' | 'sales_order' | 'delivery' | 'ar_invoice'
        and metadata for each stage reached.
        """
        try:
            if not sap_doc_entry:
                if pos_order_id:
                    pos = request.env['pos.perfume.order'].sudo().browse(pos_order_id)
                    sap_doc_entry = pos.sap_doc_entry
                if not sap_doc_entry:
                    return {'success': False, 'error': 'No sap_doc_entry provided'}

            backend = request.env['sap.backend'].sudo().search(
                [('active', '=', True)], limit=1
            )
            if not backend:
                return {'success': False, 'error': 'No active SAP backend'}
            ...
            result = {
                ...
                'stage': 'quotation',        # مرحلة أدنى افتراضية
                'quotation': None,
                'sales_order': None,
                'deliveries': [],
                'ar_invoices': [],
            ...
            # ── 1. هل هو Sales Order أم Quotation في SAP؟ ───────────────────
            r_order = conn.session.get(
                f"{base_url}/Orders({sap_doc_entry})"
                f"?$select=DocEntry,DocNum,DocDate,DocumentStatus,DocTotal",
                headers=headers, timeout=10, verify=False
            )
            r_quote = conn.session.get(
                f"{base_url}/Quotations({sap_doc_entry})"
                f"?$select=DocEntry,DocNum,DocDate,DocumentStatus,DocTotal",
                headers=headers, timeout=10, verify=False
            )
            ...
            if r_quote.status_code == 200:
                ...
                result['quotation'] = {
                    ...
                    'date': str(d.get('DocDate', ''))[:10],
                    'status': d.get('DocumentStatus'),
                    'total': d.get('DocTotal'),
                }
                result['stage'] = 'quotation'
            ...
            if r_order.status_code == 200:
                ...
                result['stage'] = 'sales_order'
                order_entry = d.get('DocEntry')

                # ── 2. Deliveries مرتبطة بالـ Sales Order ────────────────────
                r_del = conn.session.get(
                    f"{base_url}/DeliveryNotes"
                    f"?$select=DocEntry,DocNum,DocDate,DocumentStatus,DocTotal,DocumentLines"
                    f"&$orderby=DocEntry desc&$top=50",
                    headers=headers, timeout=15, verify=False
                )
                delivery_entries = set()
                if r_del.status_code == 200:
                    for doc in r_del.json().get('value', []):
                        for line in doc.get('DocumentLines', []):
                            if line.get('BaseType') == 17 and line.get('BaseEntry') == order_entry:
                                de = doc['DocEntry']
                                if de not in delivery_entries:
                                    delivery_entries.add(de)
                                    result['deliveries'].append({
                                        'doc_entry': de,
                                        'doc_num': doc.get('DocNum'),
                                        'date': str(doc.get('DocDate', ''))[:10],
                                        'status': doc.get('DocumentStatus'),
                                        'total': doc.get('DocTotal'),
                                    })
                                break
                ...
                # ── 3. AR Invoices مرتبطة بالـ Deliveries ────────────────────
                if delivery_entries:
                    r_inv = conn.session.get(
                        f"{base_url}/Invoices"
                        f"?$select=DocEntry,DocNum,DocDate,DocumentStatus,DocTotal,DocumentLines"
                        f"&$orderby=DocEntry desc&$top=50",
                        headers=headers, timeout=15, verify=False
                    ...
                    if r_inv.status_code == 200:
                        for doc in r_inv.json().get('value', []):
                            for line in doc.get('DocumentLines', []):
                                if line.get('BaseType') == 15 and line.get('BaseEntry') in delivery_entries:
                                    de = doc['DocEntry']
                                    if not any(x['doc_entry'] == de for x in result['ar_invoices']):
                                        result['ar_invoices'].append({
                                            'doc_entry': de,
                                            'doc_num': doc.get('DocNum'),
                                            'date': str(doc.get('DocDate', ''))[:10],
                                            'status': doc.get('DocumentStatus'),
                    ...
                    if result['ar_invoices']:
                        result['stage'] = 'ar_invoice'

            _logger.info(
                f"[SAP Chain] DocEntry={sap_doc_entry} → stage={result['stage']}, "
                f"deliveries={len(result['deliveries'])}, ar={len(result['ar_invoices'])}"
            )
            return result

        except Exception as e:
            _logger.error(f"[SAP Chain] Error: {e}", exc_info=True)
            return {'success': False, 'error': str(e)}
