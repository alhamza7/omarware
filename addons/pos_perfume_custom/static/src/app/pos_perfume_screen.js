...
export class PosPerfumeScreen extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            // SAP Document Chain
            sapChain: null,          // chain للطلب المحمَّل حالياً
            sapChainLoading: false,  // جاري الجلب
            sapChainCache: {},       // cache: sap_doc_entry -> chain (للقائمة)
    ...
    async openPreviousOrders() {
        try {
            ...
            // جلب حالة SAP لكل الطلبات المرتبطة بـ SAP (بشكل متوازٍ)
            const sapEntries = [...new Set(
                orders
                    .filter(o => o.sap_doc_entry && !this.state.sapChainCache[o.sap_doc_entry])
                    .map(o => o.sap_doc_entry)
            )];
            if (sapEntries.length > 0) {
                // نجلب بالتوازي ولكن نحدّ العدد لتجنب إغراق السيرفر
                const BATCH = 5;
                for (let i = 0; i < sapEntries.length; i += BATCH) {
                    const batch = sapEntries.slice(i, i + BATCH);
                    await Promise.allSettled(batch.map(entry => this.fetchSapChain(entry)));
                }
            }
    ...
    /**
     * Fetch SAP document chain for the currently loaded order.
     * Called automatically when an order with sap_doc_entry is loaded,
     * or manually via the "فحص SAP" button.
     */
    async fetchSapChain(docEntry) {
        const entry = docEntry || this.state.currentOrder.sap_doc_entry;
        if (!entry) return;

        // استخدم الـ cache إن وُجد
        if (this.state.sapChainCache[entry]) {
            this.state.sapChain = this.state.sapChainCache[entry];
            return;
        }

        this.state.sapChainLoading = true;
        this.state.sapChain = null;
        try {
            const result = await rpc('/pos_perfume/get_sap_chain', {
                sap_doc_entry: entry,
            });
            if (result && result.success) {
                this.state.sapChain = result;
                // خزِّن في الـ cache
                this.state.sapChainCache = {
                    ...this.state.sapChainCache,
                    [entry]: result,
                };
            } else {
            ...
            console.warn('[SAP Chain] fetch error:', e);
            this.state.sapChain = null;
        } finally {
            this.state.sapChainLoading = false;
        }
    }
    ...
    async loadOrder(orderId) {
        try {
            ...
            // جلب حالة SAP تلقائياً إن كان الطلب مرتبطاً بـ SAP
            if (order.sap_doc_entry) {
                this.fetchSapChain(order.sap_doc_entry);
            } else {
                this.state.sapChain = null;
                this.state.sapChainLoading = false;
            }
