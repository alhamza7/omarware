...
# Try to import from queue_job if available, otherwise use local definitions
try:
    from odoo.addons.queue_job.exception import JobError, RetryableJobError
except ImportError:
    # Local fallback definitions when queue_job is not installed
    class JobError(Exception):
        """Base class for job errors"""
        pass
    
    class RetryableJobError(JobError):
        """Job error that can be retried"""
        pass
