...
def create_sap_data_success():
    """الحل الناجح: إنشاء بيانات SAP"""
    try:
        ...
        print("=== الحل الناجح: إنشاء بيانات SAP ===")
        ...
        # 3. Create products with UoM (without uom_po_id)
        ...
        for product_name, sap_code in sample_products:
            try:
                ...
                if existing:
                    ...
                    # Create product with UoM (only uom_id)
                    cur.execute("""
                        INSERT INTO product_template (
                            name, type, uom_id, create_uid, write_uid, create_date, write_date
                        ) VALUES (%s, %s, %s, %s, %s, NOW(), NOW())
                        RETURNING id;
                    """, (f'{{"en_US": "{product_name}"}}', 'product', default_uom_id, 2, 2))
...
if __name__ == "__main__":
    success = create_sap_data_success()
    if success:
        print("\n🎉 تمت المزامنة بنجاح!")
