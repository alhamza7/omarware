...
print(f"🧪 اختبار: إنشاء مستند في فولدر له company_id\n")

with Registry(db_name).cursor() as cr:
    env = api.Environment(cr, SUPERUSER_ID, {})
    
    # Get folder with company_id
    ...
    cr.commit()  # حفظ في قاعدة البيانات
    
    print(f"✅ تم إنشاء المستند: {doc.name} (ID: {doc.id})")
    print(f"\n📊 معلومات المستند:")
    ...
    if doc.company_id and doc.company_id.id == folder.company_id.id:
        print(f"\n✅✅✅ نجح! المستند ورث company_id من الفولدر!")
    else:
        print(f"\n❌ فشل! المستند لم يحصل على company_id")
    
    print(f"\n🔍 الآن اختبر عبر API:")
    print(f"   GET /api/documents/{doc.id}")
    print(f"   يجب أن ترى: company_id = {folder.company_id.id}, company_name = '{folder.company_id.name}'")
